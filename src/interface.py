...
class Interface():
    ...
    def finalizarCompra(self, forma_pagamento:int) -> None:
        ...
        print("Validando pagamento", end="", flush=True)
        sleep(1)
        ...
        print(".", flush=True)
        sleep(1)
        print("Pagamento confirmado.", flush=True)

    def dispensarBebidaLata(self, quantidade:int, bebida:str):
        print(f"\nDispensando {quantidade} {bebida}", end="", flush=True)
        if quantidade > 1:
    ...
    def dispensarIngrediente(self, doses:list[int], ingredientes:list[str]):
        ...
        print("\nBebida dosada dispensada. Aproveite!", flush=True)
        self.estadoEspera()