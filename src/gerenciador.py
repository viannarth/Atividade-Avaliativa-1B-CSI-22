...
class GerenciadorMaquina():
    ...
    def executarEstado(self) -> None:
        if self._estado == Estado.TELA_INICIAL:
            ...
            self.escolhaBebida()
        elif self._estado == Estado.ADICIONAR_BEBIDA:
    ...
    def escolhaBebida(self) -> None:
        ...
        if carrinho_vazio:
            if input_ == 1:
                self.alterarEstado(Estado.ADICIONAR_BEBIDA)
            elif input_ == 2:
    ...
    def adicionarBebidaLata(self) -> None:
        input_:str | ValueError = self._interface.verificarBebidaLata()
        bebida_lata = self._maquina.verificarBebidaLata(input_)
        if input_ == ValueError or not bebida_lata:
            self._interface.opcaoInvalida()
            return
        else:
            self._maquina.adicionarBebida()
